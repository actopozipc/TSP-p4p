...
class SA: #SimulatedAnnealing
    def __init__(self, towns, startTemp=10, cycles=100):
    ...
    def GenerateNewConfiguration(self, S):
        ...
        energyDiffernece = ts.energyOfTwoTowns(self.towns[amin], self.towns[b]) + ts.energyOfTwoTowns(
            self.towns[a], self.towns[bmax]) - ts.energyOfTwoTowns(self.towns[amin], self.towns[a]) - ts.energyOfTwoTowns(self.towns[b], self.towns[bmax])
    ...
    def Update(self):
        ...
        for i in range(1,self.cycles,1):
            if len(self.allPaths) == math.factorial(len(self.towns)): #Falls schon alle Kombinationen generiert, hör auf zu suchen
                return bestPath;
            ...
            if newConfiguration not in self.allPaths: #falls neuer Pfad noch nicht generiert wurde
                self.allEnergies.append(newEnergy) #speichere neue Energie
                self.allPaths.append(newConfiguration[:]) #speichere neuen Pfad
    ...
    def AcceptanceProbability(self, newEnergy, sa, newConfiguration, oldConfiguration):
        ...
        probability = math.exp(-deltaE / sa.currentTemp) ##self.Boltzmann(self.towns, self.currentTemp, self.path, deltaE);
        acceptanceProbability = sa.Boltzmann(sa.towns, sa.currentTemp, newConfiguration, newEnergy) / sa.Boltzmann(sa.towns, sa.currentTemp, oldConfiguration, sa.currentEnergy );
